	...
	class RevistaController extends Revista{	
		...
		public function insertaRevista($datos,$archivos){
			...
			if(count($this->errores)>0){
				...
				//print_r($this->errores);//comentar esto
				$this->muestra_errores = true;
				$er1=new ER();
				$er1->manda_errores($errores,$muestra_errores);
				//die();//detieene o mata todo el script.
			}else{
				//mueve la imagen
				//move_uploaded_file($archivos['portada']['tmp_name'],//ruta origen
				//"../images/".$archivos['portada']['name']);//ruta destino
